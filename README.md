...
Version 0.2.0 introduced caching of the visible scene during panning, continuous zooming, and commit transitions. Hover draws a separate overlay. On three real Linux snapshots at 1100×850 / DPR 2 in Chromium without CPU throttling, panning frame-interval p95 improved from 27.8 to 9.2 ms and zooming from 32.5 to 9.6 ms; painting JavaScript p95 dropped from 3.9/5.1 ms to 0.2/0.3 ms. Those interaction runs had no tasks over 50 ms. A new commit's initial scene rasterization still costs about 39 ms before its cached transition. Version 0.2.1 fixes the large-screen blur caused by the original 4M-pixel cap: static rendering keeps native DPR up to 12M pixels per surface and 8192 pixels per side, while gestures use a prepared preview and display backing store of at most 4M pixels. Overscan shrinks before detail is reduced. Display, scene caches, and previews share a 32M-pixel budget (128 MiB of RGBA storage); obsolete caches are released before allocating replacements, and a transition is skipped when two scenes would exceed this budget. Browser/GPU overhead is additional. Hidden and unmounted views release their caches.

Each snapshot now owns a deterministic grid with at most 620 blocks, shared by Overview and Inspect. Selection changes highlighting only; a file outside the visible sample focuses its directory. Removing interpolation between incompatible old and new slots eliminates overlapping buildings and accumulated drift when seeking backwards.
